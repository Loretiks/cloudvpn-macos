...
- **Homebrew (рекомендуется, без ручного обхода Gatekeeper):**
  ...
  brew install --cask cloudvpn        # свежий brew попросит: brew trust loretiks/cloudvpn
  ```
  brew сам снимает карантин — приложение запускается сразу. Cask: `Casks/cloudvpn.rb`
  (живёт в отдельном tap-репо `Loretiks/homebrew-cloudvpn`; `release.sh` обновляет в нём
  version+sha256). Свежие версии Homebrew при первом обращении к стороннему tap просят
  подтвердить доверие — `brew trust loretiks/cloudvpn`.
